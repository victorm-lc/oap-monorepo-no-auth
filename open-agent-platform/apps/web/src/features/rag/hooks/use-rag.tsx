...
export function useRag(): UseRagReturn {
  ...
  const initialFetch = useCallback(async (accessToken: string) => {
    ...
    try {
      // For demo mode, use a demo token instead of the passed accessToken
      const demoToken = "user1";
      initCollections = await getCollections(demoToken);
    } catch (e: any) {
      console.error("Failed to fetch collections:", e);
      // Skip database initialization since it doesn't exist in langconnect
      // Just proceed with empty collections
    }
    ...
    const documents = await listDocuments(
      ...
      "user1", // Use demo token for demo mode
    );
  ...
  const listDocuments = useCallback(
    async (
      ...
      // For demo mode, always use demo token
      const demoToken = "user1";
  ...
  const handleTextUpload = useCallback(
    async (textInput: string, collectionId: string) => {
      ...
      await uploadDocuments(collectionId, [textFile], demoToken, [
        metadata,
      ]);
      setDocuments((prevDocs) => [
        ...prevDocs,
